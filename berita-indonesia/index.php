...
<html lang="en">
  <head>
    ...
    <title>Berita Indonesia - Sport Zone</title>
		...
		<div class="row">
		  <div class="col-md-12" style="background:white;height:200px auto;">
			<div class="row">
				...
				$total = mysql_query("SELECT * FROM berita WHERE kategori='Indonesia'");
				...
				<?php
					$query = mysql_query("SELECT * FROM berita WHERE kategori='Indonesia' ORDER BY tanggal DESC LIMIT $awal, $hal");
					while($row = mysql_fetch_array($query)){
				?>
				</div>
				<div class="media">
				...
				<nav>
				  <ul class="pagination">
					<li>
						<?php if($awal != 0){ echo "<a href='../berita-indonesia/?awal=".($awal-$hal)."' aria-label='Previous'>
					...
					<li>
						<?php for($i=0; $i <  $jum_hal; $i++){
							$x = $i * $hal;
							if($awal == $x){ echo "<span aria-hidden='true' class='disabled' style='color:black;'>".($i+1)."</span>"; }else{ echo" <a href='../berita-indonesia/?awal=$x'>
							".($i+1)."
							</a>";
							}
							}
					...
					<li>
						<?php if($awal != $x){ echo "<a href='../berita-indonesia/?awal=".($awal+$hal)."' aria-label='Next'>
