		...
		<div class="row">
		  <div class="col-md-12" style="background:white;height:200px auto;">
			<div class="row">
				...
				$kata = @$_POST['cari'];
				$total = mysql_query("SELECT * FROM berita WHERE judul like '%$kata%' OR isi like '%$kata%'");
				...
				<nav>
				  <ul class="pagination">
					<li>
						<?php if($awal != 0){ echo "<a href='../cari/?awal=".($awal-$hal)."' aria-label='Previous'>
					...
					<li>
						<?php for($i=0; $i <  $jum_hal; $i++){
							$x = $i * $hal;
							if($awal == $x){ echo "<span aria-hidden='true' class='disabled' style='color:black;'>".($i+1)."</span>"; }else{ echo" <a href='../cari/?awal=$x'>
							".($i+1)."
							</a>";
							}
							}
					...
					<li>
						<?php if($awal != $x){ echo "<a href='../cari/?awal=".($awal+$hal)."' aria-label='Next'>
